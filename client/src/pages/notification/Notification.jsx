...
const Notification = () => {
   ...
   useEffect(()=>{
    ...
    const getnot=async()=>
    {
        const res=await axios.get(`http://localhost:8000/getNotify/${FacebookUserId}`)
        setNotification(res.data.sort((p1, p2) => {
              ...
              <div className="row">
                  {
                      notification?.map((val,ind)=>{
                          return(
                              <>
                              <div className="col-md-10 col-12 mx-auto mb-3" key={ind} onClick={()=>his.push(`/postdetails/${val.post_id}`)}>
                        <div className="card p-3">
                        <p> <strong>{val.name}</strong> messaged <strong>{val.message}</strong> , Click here to see this post</p>
                        </div>
                    </div>
