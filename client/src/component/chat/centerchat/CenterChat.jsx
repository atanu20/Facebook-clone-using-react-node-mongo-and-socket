...
const CenterChat = ({conversationmsg,conversationId,setNewMessage,newMessage,setConversationMsg }) => {
    const socket = useRef();
...
const getConDetails=async()=>{
    const res=await axios.get(`http://localhost:8000/conversationid/${conversationId}`)
    ...
    const handleSubmit=async(e)=>{
        ...
        try {
            const res = await axios.post("http://localhost:8000/sendconv", message);
    ...
    return (
       ...
       <div className="centerChatbox">
           ...
           <div className="centerChatboxmsg">
               {
                   conversationmsg.map((m,ind)=>{
                       return(
                           <>
                           <div ref={scrollRef}>
                           <Message key={ind} friendId={friendId} message={m} own={m.senderId === FacebookUserId} />
                           </div>
                           </>
                       )
                   })
           ...
           <div className="centerChatboxmsgwrite p-2">
                ...
                <input type="text" className="inputbox" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} autoComplete="off" required />
