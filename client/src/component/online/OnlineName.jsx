...
const OnlineName = ({profilePicture,userID,getConv}) => {
    ...
    const getmyidname=async()=>{
      ...
      const conversationroom=async()=>{
        ...
        let res=await axios.post(`http://localhost:8000/conversationroom`,data);
        notify(res.data.msg)
        if(getConv)
        {
          getConv()
    ...
    return (
      ...
      <div className="rightbarProfileImgContainer">
        <img className="rightbarProfileImg" src={profilePicture ? `../profile/${profilePicture}` : `../assets/pro.png`} alt="" />
        <span className="rightbarOnline"></span>
      </div>
      <span className="rightbarUsername">{pname}</span>
      </div>
      <button className="btn btn-primary btn-sm" onClick={conversationroom}>Start</button>
